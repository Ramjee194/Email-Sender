// src/services/schedulerService.ts - FIXED TIMER TYPE
...
class SchedulerService {
  private db: Database;
  private schedulerInterval: number | null = null; // ✅ Changed Timer to number
  ...
  private initDatabase() {
    ...
    try {
      ...
      if (!error.message.includes("duplicate column name")) {
        console.log("Note: user_id column already exists");
      }
    }
    ...
    try {
      ...
      // Column already exists, ignore
      if (!error.message.includes("duplicate column name")) {
        console.log("Note: config_name column already exists");
      }
    }
  ...
  private async monitorBatchJobCompletionOnly(jobId: string) {
    const checkCompletion = async () => {
      try {
        ...
        console.error(`❌ Error monitoring batch job ${jobId}:`, error);
      }
    };
  ...
  getUserScheduledJobs(userId: string): any[] {
    try {
      ...
      console.error("❌ Error getting user scheduled jobs:", error);
      return [];
    }
  }
}
