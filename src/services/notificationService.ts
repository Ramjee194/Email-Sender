...
import type { userDatabase, UserSMTPConfig } from "./userDatabase";
...
class NotificationService {
  ...
  getCampaignStats(jobId: string): JobStats {
    ...
    const campaignLogs = logs.filter((log:any) => log.id.includes(jobId));
    ...
    const errors = campaignLogs.filter((log:any) => log.status === "Error").length;
